#!/usr/bin/env python3
"""
send_slack_exp.py — Slack notification for sweep/experiment completion.

Python API:
    from send_slack_exp import notify_sweep_done
    notify_sweep_done("MINT-TTA lmi=5 shard1", summary="best=0.712, 10 corruptions", elapsed=3600)

Shell usage:
    python send_slack_exp.py "Shard 1 DONE" "best_acc=0.712"
    python send_slack_exp.py "Shard 1 DONE" "best_acc=0.712" --elapsed 3600
"""
...
def _post(text: str) -> None:
    if not SLACK_TOKEN:
        print(f"[send_slack_exp] SLACK_TOKEN 미설정 — 알림 생략\n{text}")
        return
    try:
        resp = requests.post(
        ...
        data = resp.json()
        if data.get("ok"):
            print("[send_slack_exp] Slack 알림 전송 성공")
        else:
            print(f"[send_slack_exp] 전송 실패: {data.get('error')}")
    except Exception as e:
        print(f"[send_slack_exp] 예외 발생: {e}")
...
def notify_sweep_done(
    ...
    Args:
        sweep_name: 알림 제목 (예: "MINT corruption sweep lmi=5 shard1").
    ...
    lines = [f":white_check_mark: *Sweep 완료: {sweep_name}*"]
    if summary:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start",    default=None,
                        help="시작 시각 문자열 (예: '2026-03-06 21:32:58')")
