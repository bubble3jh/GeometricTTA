...
BATCLIP_PER_CORRUPTION = {
    ...
    "fog":               0.8156,   # shard4 (100-18.44%)
    "brightness":        0.8826,   # shard4 (100-11.74%)
    "contrast":          0.8084,   # shard4 (100-19.16%)
    "elastic_transform": 0.6843,   # shard5 (100-31.57%)
    "pixelate":          0.6478,   # shard5 (100-35.22%)
    "jpeg_compression":  None,     # shard6에서 측정 예정
}
...
def run_mint(label, model, state_init, all_data, device, *,
             ...
             use_entropy=True,
             w_i2t=1.0, alpha_s=2.0,
             # ── Gap 6: I2T weight mode ──
             use_uniform_i2t=False,
             # ── CALM v2: Adaptive λ ──
             lambda_mode="fixed",     # "fixed" | "adaptive_A" | "adaptive_B"
             alpha_adapt=0.0,         # Variant B scaling factor
             ):
    model.load_state_dict(state_init)
    ...
    lambda_t_list, collapse_list = [], []    # CALM v2 adaptive λ tracking
    cumulative_correct, cumulative_seen = 0, 0
    ...
    for step, (imgs_b, labels_b) in enumerate(all_data):
        ...
        # ── H: Total Loss (with adaptive λ support) ─────────────────────────
        if lambda_mode == "adaptive_A" and lambda_mi > 0:
            log_K = torch.log(torch.tensor(float(K), device=device))
            collapse_score = (log_K - l_hy).detach()
            lambda_t = lambda_mi * (1.0 + collapse_score / log_K)
        elif lambda_mode == "adaptive_B" and lambda_mi > 0:
            log_K = torch.log(torch.tensor(float(K), device=device))
            collapse_score = (log_K - l_hy).detach()
            lambda_t = lambda_mi + alpha_adapt * collapse_score
        else:
            lambda_t = lambda_mi
            collapse_score = None

        loss = l_ent - lambda_t * l_hy + w_cov * l_cov - w_i2t * l_i2t
        ...
        # CALM v2: track adaptive λ
        with torch.no_grad():
            lt_val = float(lambda_t) if isinstance(lambda_t, (int, float)) else float(lambda_t.item()) if hasattr(lambda_t, 'item') else float(lambda_t)
            cs_val = float(collapse_score.item()) if collapse_score is not None else 0.0
        lambda_t_list.append(lt_val)
        collapse_list.append(cs_val)

        # Track cumulative (overall) accuracy
        cumulative_correct += int((preds == labels_b).sum().item())
        cumulative_seen += B

        if (step + 1) % 10 == 0:
            lt_str = f" λ_t={lt_val:.3f}" if lambda_mode != "fixed" else ""
            logger.info(f"  [{label}] step {step+1:2d}/{N_STEPS} "
                        f"acc={acc:.3f} sink={sink_frac:.3f} "
                        f"H(Y)={h_y:.3f} d_eff={d_eff:.2f}{lt_str}")

    final_acc = float(np.mean(acc_list[-5:]))
    overall_acc = cumulative_correct / cumulative_seen if cumulative_seen > 0 else 0.0
    logger.info(f"  [{label}] DONE last5={final_acc:.4f} overall={overall_acc:.4f} "
    ...
    return {
        ...
        "lambda_t_profile":   lambda_t_list,
        "collapse_profile":   collapse_list,
        "config": {
...
def main():
    ...
    for label, use_pc, l_adj, l_mi, use_ent in ph1_conditions:
        ...
        r = run_mint(label, model, model_state_init, corr_data, device,
                     ...
                     gamma_norm=0.0, w_cov=0.0, w_i2t=1.0)
        ph1_runs.append(r)
    ...
    if scenario in ("A", "B"):
        ...
        for label, use_pc, l_mi, use_wm in ph2_conditions:
            ...
            r = run_mint(label, model, model_state_init, corr_data, device,
                         ...
                         tau_inf=0.0, gamma_norm=0.0, w_cov=0.0, w_i2t=1.0)
            ph2_runs.append(r)
    ...
    # ════════════════════════════════════════════════════════════════════════
    logger.info("\n" + "─"*60)
    logger.info("Phase 4: L_cov sweep")
    logger.info("─"*60)

    ph4_conditions = [
        # (label,           w_cov, w_i2t)
        ("ph3_best",        0.5,   1.0),
        ("barlow_cov_01",   0.1,   1.0),
        ("barlow_cov_05",   0.5,   1.0),
        ("barlow_cov_10",   1.0,   1.0),
        ("no_i2t",          0.5,   0.0),   # I2T 필요성 검증
    ]

    ph4_runs = []
    for label, w_cov, wi2t in ph4_conditions:
        logger.info(f"\n  Running: {label}")
        r = run_mint(label, model, model_state_init, corr_data, device,
                     use_prior_correction=False,
                     lambda_mi=best_lmi, use_weighted_marginal=best_wm3,
                     tau_inf=best_tau, use_entropy=best_ent,
                     w_cov=w_cov, gamma_norm=0.0, w_i2t=wi2t)
        ph4_runs.append(r)

    best_ph4 = print_phase_summary("Phase 4: L_cov", ph4_runs)
    all_results["phases"]["phase4"] = {"runs": ph4_runs, "best": best_ph4["label"]}

    best_wcov  = best_ph4["config"]["w_cov"]
    ...
    if phase5_enable:
        ...
        for label, g_norm in ph5_conditions:
            ...
            r = run_mint(label, model, model_state_init, corr_data, device,
                         ...
                         lambda_mi=best_lmi, use_weighted_marginal=best_wm3,
                         tau_inf=best_tau, use_entropy=best_ent,
                         w_cov=best_wcov, gamma_norm=g_norm, w_i2t=best_wi2t4)
            ph5_runs.append(r)
