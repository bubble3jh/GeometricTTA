...
def main():
    ...
    gap1_conditions = [
        # (label,              w_cov)
        ("barlow_cov_01_ref",  0.1),   # Phase 4 best (was use_bv=True, λ_cov=0.1)
        ("cov01_no_var",       0.1),   # w_cov=0.1, no L_var (both identical — confirms L_var=0)
    ]

    gap1_runs = []
    for label, w_cov in gap1_conditions:
        logger.info(f"\n  Running: {label}")
    ...
    for label, use_uni_i2t in gap6_conditions:
        ...
        r = run_mint(label, model, model_state_init, corr_data, device,
                     ...
                     tau_inf=BEST_TAU,
                     use_entropy=BEST_ENT,
                     w_cov=BEST_WCOV, gamma_norm=0.0,
                     w_i2t=BEST_WI2T,
