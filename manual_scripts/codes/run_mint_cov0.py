...
def main():
    ...
    conditions = [
        # (label,              w_cov, use_uniform_i2t)
        ("cov0_uniform_i2t",   0.0,   True),    # w_cov=0, uniform I2T
        ("cov01_uniform_i2t",  0.1,   True),    # w_cov=0.1, uniform I2T (best so far, re-run)
    ]

    runs = []
    for label, w_cov, uni_i2t in conditions:
        logger.info(f"\nRunning: {label}  (w_cov={w_cov}, uniform_i2t={uni_i2t})")
