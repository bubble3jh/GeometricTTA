...
Grid (per call):
  ...
  λ_cov   : {0, 0.1}
  w_i2t   : {0.0, 1.0}
  Fixed   : use_entropy=True, use_uniform_i2t=True, use_barlow_var=False,
...
FIXED = dict(
    ...
    w_uni                  = 0.0,      # covariance penalty uses w_uni path; kept 0 when λ_cov given
    alpha_s                = 2.0,
)

# λ_cov × w_i2t grid
GRID = [
    # (label_suffix,  lambda_cov, w_i2t)
    ("cov0_i2t0",     0.0,        0.0),
    ("cov0_i2t1",     0.0,        1.0),
    ("cov01_i2t0",    0.1,        0.0),
    ("cov01_i2t1",    0.1,        1.0),
...
def run_one_corruption(corruption, lambda_mi, model, model_state_init, device, preprocess):
    ...
    results = []
    for suffix, lambda_cov, w_i2t in GRID:
        label = f"{corruption}_lmi{lambda_mi}_{suffix}"
        logger.info(f"\n  → {label}  (λ_cov={lambda_cov}, w_i2t={w_i2t})")
        r = run_mint(
...
def main():
    ...
    all_results = {
        ...
        "grid":        [{"suffix": s, "lambda_cov": c, "w_i2t": w} for s, c, w in GRID],
